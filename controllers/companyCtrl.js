...
exports.createCompany = async (req, res) => {
  if (
    ...
    return res
      .status(200)
      .json({ error: "You cannot create company with empty fields" });
  }
...
exports.search = async (req, res) => {
  ...
  if (company.length > 0) {
    return res
      .status(200)
      .json({ message: "Search Results", results: company });
  } else {
...
exports.leaderBoard = async (req, res) => {
  const results = await Company.find({}).sort({ totalStars: -1 });
